Vue.component('Copydata', {
	template: `
		<el-dialog title="复制添加" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
				<el-row >
					<el-col :span="24" style="display: none">
						<el-form-item label="超级管理" prop="root">
							<el-input  v-model="form.root" autoComplete="off" clearable  placeholder="请输入超级管理"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row >
			...
			if(val){
				axios.post(base_url + '/ShopAdmin/getFieldList').then(res => {
					if(res.data.status == 200){
						...
						// this.gwgl_ids = res.data.data.gwgl_ids
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/ShopAdmin/copydata',this.form).then(res => {
						if(res.data.status == 200){
