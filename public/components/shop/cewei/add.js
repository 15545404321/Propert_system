	...
	template: `
		<el-drawer title="添加"  direction="rtl" size="1220px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			...
			<el-tabs v-model="activeName">
				...
				<el-tab-pane style="padding-top:10px"  label="批量添加" name="批量添加">
				<el-row >
					<el-col :span="24">
						<el-form-item label="批量添加" prop="cw_pltj">
							<el-radio-group v-model="form.cw_pltj">
				...
				<el-row v-if="form.cw_pltj == 2">
					<el-col :span="24">
						<el-form-item label="新增数量" prop="cw_num">
							<el-input-number controls-position="right" style="width:200px;" autoComplete="off" v-model="form.cw_num" clearable :min="0" placeholder="请输入新增数量"/>
						</el-form-item>
					</el-col>
				</el-row>
				</el-tab-pane>
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Cewei/add',this.form).then(res => {
						if(res.data.status == 200){
