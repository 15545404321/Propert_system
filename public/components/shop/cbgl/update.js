Vue.component('Update', {
	...
	methods: {
		open(){
			this.form = this.info
			this.form.cbgl_kstime = parseTime(this.form.cbgl_kstime)
			this.form.cbgl_jstime = parseTime(this.form.cbgl_jstime)
			if(this.info.pid == '0' ){
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Cbgl/update',this.form).then(res => {
						if(res.data.status == 200){
