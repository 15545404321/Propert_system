Vue.component('Update', {
	...
	methods: {
		open(){
			this.form = this.info
			this.form.yssj_kstime = parseTime(this.form.yssj_kstime)
			this.form.yssj_jztiem = parseTime(this.form.yssj_jztiem)
			this.form.yssj_fksj = parseTime(this.form.yssj_fksj)
		},
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Yssj/update',this.form).then(res => {
						if(res.data.status == 200){
