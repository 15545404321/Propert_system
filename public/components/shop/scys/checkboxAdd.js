Vue.component('Checkboxadd', {
	template: `
		<el-dialog title="多选生成应收" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
	...
	methods: {
		open(){
			this.strictly = true
			axios.post(base_url+'/Scys/getRoleAccessAdd').then(res => {
				if(res.data.status == 200){
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					...
					axios.post(base_url + '/Scys/checkboxAdd',this.form).then(res => {
						if(res.data.status == 200){
