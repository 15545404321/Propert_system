Vue.component('Add', {
	template: `
		<el-dialog title="生成应收" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Scys/add',this.form).then(res => {
						if(res.data.status == 200){
