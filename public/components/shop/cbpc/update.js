Vue.component('Update', {
	...
	methods: {
		open(){
			this.form = this.info
			this.form.cbpc_kstime = parseTime(this.form.cbpc_kstime)
			this.form.cbpc_jstime = parseTime(this.form.cbpc_jstime)
		},
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Cbpc/update',this.form).then(res => {
						if(res.data.status == 200){
