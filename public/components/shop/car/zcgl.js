Vue.component('Zcgl', {
	template: `
		<el-dialog title="资产关联" width="800px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
	...
	watch:{
		show(val){
			if(this.info.member_id && val){
				axios.post(base_url + '/Car/remoteMemberidList',{dataval:this.info.member_id}).then(res => {
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Car/zcgl',this.form).then(res => {
						if(res.data.status == 200){
