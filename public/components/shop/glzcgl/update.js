Vue.component('Update', {
	template: `
		<el-dialog title="修改" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
				...
				<el-row >
					<el-col :span="24">
						<el-form-item label="操作时间" prop="glzcgl_time">
							<el-date-picker value-format="yyyy-MM-dd HH:mm:ss" type="date" v-model="form.glzcgl_time" clearable placeholder="请输入操作时间"></el-date-picker>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
	...
	watch:{
		show(val){
			if(val){
				axios.post(base_url + '/Glzcgl/getFcxx_id',{louyu_id:this.info.louyu_id}).then(res => {
			...
			if(val){
				axios.post(base_url + '/Glzcgl/getCewei_id',{tccd_id:this.info.tccd_id}).then(res => {
					if(res.data.status == 200){
	...
	methods: {
		open(){
			...
			this.setDefaultVal('glzcgl_type')
			this.form.glzcgl_time = parseTime(this.form.glzcgl_time)
		},
		selectFcxx_id(val){
			this.form.fcxx_id = ''
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Glzcgl/update',this.form).then(res => {
						if(res.data.status == 200){
