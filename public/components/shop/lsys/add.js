Vue.component('Add', {
	template: `
		<el-dialog title="临时应收" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
	...
	methods: {
		open(){
			this.strictly = true
			axios.post(base_url+'/Lsys/getRoleAccessAdd').then(res => {
				if(res.data.status == 200){
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					...
					axios.post(base_url + '/Lsys/add',this.form).then(res => {
						if(res.data.status == 200){
