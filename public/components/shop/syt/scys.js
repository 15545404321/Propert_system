Vue.component('Scys', {
	template: `
		<el-drawer title="生成应收"  direction="rtl" size="1200px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
				...
				<el-row >
					<el-col :span="24">
						<el-form-item label="单元编号" prop="danyuan_id">
							<el-select   style="width:100%" v-model="form.danyuan_id" filterable clearable placeholder="请选择楼宇单元">
	...
	methods: {
		...
		selectDanyuan_id(val){
			this.form.danyuan_id = ''
			axios.post(base_url + '/Syt/getDanyuan_id',{louyu_id:val}).then(res => {
				if(res.data.status == 200){
		...
		selectFcxx_id(val){
			this.form.fcxx_id = ''
			axios.post(base_url + '/Syt/getFcxx_id',{danyuan_id:val}).then(res => {
				if(res.data.status == 200){
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Syt/submitScys',this.form).then(res => {
						if(res.data.status == 200){
