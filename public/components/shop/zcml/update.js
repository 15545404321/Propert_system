Vue.component('Update', {
	template: `
		<el-drawer title="修改"  direction="rtl" size="1000px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
	...
	watch:{
		show(val){
			if(val){
				axios.post(base_url + '/Zcml/getZclb_id',{zclb_fid:this.info.zclb_fid}).then(res => {
	...
	methods: {
		open(){
			this.form = this.info
			this.form.zcml_time = parseTime(this.form.zcml_time)
			this.setDefaultVal('zcml_pic')
			this.setDefaultVal('zcml_fj')
		},
		selectZclb_id(val){
			this.form.zclb_id = ''
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Zcml/update',this.form).then(res => {
						if(res.data.status == 200){
