Vue.component('Update', {
				...
				<el-row>
					<el-col :span="24">
						<el-form-item label="岗位权限" prop="gwgl_role">
							<el-tree class="tree-border" @check="checkStrictly" :data="options" show-checkbox ref="menu" node-key="access"  :default-checked-keys="form.gwgl_role" :check-strictly="strictly" empty-text="加载中，请稍后" :props="defaultProps"></el-tree>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
	...
	watch:{
		show(val){
			if(val){
				axios.post(base_url + '/Gwgl/getGwgl_sjgw',{xqgl_id:this.info.xqgl_id}).then(res => {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					...
					axios.post(base_url + '/Gwgl/update',this.form).then(res => {
						if(res.data.status == 200){
