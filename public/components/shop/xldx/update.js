Vue.component('Update', {
	...
	watch:{
		show(val){
			if(val){
				axios.post(base_url + '/Xldx/getFybz_id',{fydy_id:this.info.fydy_id}).then(res => {
			...
			if(this.info.cewei_id && val){
				axios.post(base_url + '/Xldx/remoteCeweiidList',{dataval:this.info.cewei_id}).then(res => {
					if(res.data.status == 200){
	...
	methods: {
		open(){
			this.form = this.info
			this.form.cwfy_kstime = parseTime(this.form.cwfy_kstime)
			this.form.cwfy_zztime = parseTime(this.form.cwfy_zztime)
			this.setDefaultVal('dxxx')
			this.setDefaultVal('duoxuan')
		},
		selectFybz_id(val){
			this.form.fybz_id = ''
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Xldx/update',this.form).then(res => {
						if(res.data.status == 200){
