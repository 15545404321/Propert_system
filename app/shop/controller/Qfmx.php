...
/*
 module:		欠费明细控制器
 create_time:	2023-02-17 15:50:28
 author:		
 contact:		
*/
...
use app\shop\model\Qfmx as QfmxModel;
use think\facade\Db;

class Qfmx extends Admin {
...
";
	...
	function delete(){
		...
		if($ret = hook('hook/Qfmx@beforShopDelete',$idx)){
			return $ret;
		}

		QfmxModel::destroy(['yssj_id'=>explode(',',$idx)],true);

		if($ret = hook('hook/Qfmx@afterShopDelete',$idx)){
			return $ret;
		}
	...
	function cxsc(){
		...
		if($ret = hook('hook/Qfmx@beforShopCxsc',$idx)){
			return $ret;
		}

		QfmxModel::destroy(['yssj_id'=>explode(',',$idx)],true);

		if($ret = hook('hook/Qfmx@afterShopCxsc',$idx)){
			return $ret;
		}
...
";
        ...
        if($state == 'ok'){
            ...
            $sheet->setCellValue('B1','费用名称');
            $sheet->setCellValue('C1','财务月份');
            $sheet->setCellValue('D1','开始日期');
            $sheet->setCellValue('E1','截至日期');
            $sheet->setCellValue('F1','费用单价');
            $sheet->setCellValue('G1','应收金额');
            $sheet->setCellValue('H1','费用类型');
            $sheet->setCellValue('I1','费用标准');
            $sheet->setCellValue('J1','付款状态');
            $sheet->setCellValue('K1','客户名称');

            foreach(cache($cache_key) as $k=>$v) {
                $sheet->setCellValue('A'.($k+2),$v['fcxx_fjbh']); // 房屋资产
                $sheet->setCellValue('B'.($k+2),$v['yssj_fymc']); // 费用名称
                $sheet->setCellValue('C'.($k+2),$v['yssj_cwyf']); // 财务月份
                $sheet->setCellValue('D'.($k+2),!empty($v['yssj_kstime']) ? date('Y-m-d',$v['yssj_kstime']) : ''); // 开始日期
                $sheet->setCellValue('E'.($k+2),!empty($v['yssj_jztime']) ? date('Y-m-d',$v['yssj_jztime']) : ''); // 截至日期
                $sheet->setCellValue('F'.($k+2),$v['yssj_fydj']); // 费用单价
                $sheet->setCellValue('G'.($k+2),$v['yssj_ysje']); // 应收金额
                $sheet->setCellValue('H'.($k+2),$v['fylx_name']); // 费用类型
                $sheet->setCellValue('I'.($k+2),$v['fybz_name']); // 费用标准
                $sheet->setCellValue('J'.($k+2),getItemVal($v['yssj_stuats'],'[{"key":"已付款","val":"1","label_color":"success"},{"key":"未付款","val":"0","label_color":"danger"},{"key":"已退款","val":"2","label_color":"info"},{"key":"转预存","val":"3","label_color":"primary"}]')); // 付款状态
                $sheet->setCellValue('K'.($k+2),$v['member_name']); // 客户名称
            }

            $filename = '欠费明细.'.config('my.dump_extension');
