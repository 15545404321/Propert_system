...
/*
 module:		交易明细控制器
 create_time:	2023-02-18 15:50:21
 author:		
 contact:		
*/
...
use app\shop\model\Jymx as JymxModel;
use think\facade\Db;
use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;

class Jymx extends Admin {
	...
	function initialize(){
		parent::initialize();
		if(in_array($this->request->action(),['detail','cxsc'])){
			$idx = $this->request->post('yssj_id','','serach_in');
			if($idx){
				foreach(explode(',',$idx) as $v){
					$info = JymxModel::find($v);
...
left join cd_syt as g on a.syt_id = g.syt_id
left join cd_skfs as s on g.syt_method = s.skfs_id
...
";
		...
		if($state == 'ok'){
            ...
            $sheet->setCellValue('C1','费用名称');
            $sheet->setCellValue('D1','财务月份');
            $sheet->setCellValue('E1','开始日期');
            $sheet->setCellValue('F1','截至日期');
            $sheet->setCellValue('G1','费用单价');
            $sheet->setCellValue('H1','应收金额');
            $sheet->setCellValue('I1','费用类型');
            $sheet->setCellValue('J1','费用标准');
            $sheet->setCellValue('K1','付款状态');
            $sheet->setCellValue('L1','客户名称');
            $sheet->setCellValue('M1','收款方式');

			foreach(cache($cache_key) as $k=>$v){
                $sheet->setCellValue('A'.($k+2),$v['fcxx_fjbh']); // 房屋资产
                $sheet->setCellValue('B'.($k+2),$v['cewei_name']); // 车位资产
                $sheet->setCellValue('C'.($k+2),$v['yssj_fymc']); // 费用名称
                $sheet->setCellValue('D'.($k+2),$v['yssj_cwyf']); // 财务月份
                $sheet->setCellValue('E'.($k+2),!empty($v['yssj_kstime']) ? date('Y-m-d',$v['yssj_kstime']) : ''); // 开始日期
                $sheet->setCellValue('F'.($k+2),!empty($v['yssj_jztime']) ? date('Y-m-d',$v['yssj_jztime']) : ''); // 截至日期
                $sheet->setCellValue('G'.($k+2),$v['yssj_fydj']); // 费用单价
                $sheet->setCellValue('H'.($k+2),$v['yssj_ysje']); // 应收金额
                $sheet->setCellValue('I'.($k+2),$v['fylx_name']); // 费用类型
                $sheet->setCellValue('J'.($k+2),$v['fybz_name']); // 费用标准
                $sheet->setCellValue('K'.($k+2),getItemVal($v['yssj_stuats'],'[{"key":"已付款","val":"1","label_color":"success"},{"key":"未付款","val":"0","label_color":"danger"},{"key":"已退款","val":"2","label_color":"info"},{"key":"转预存","val":"3","label_color":"primary"}]')); // 付款状态
                $sheet->setCellValue('L'.($k+2),$v['member_name']); // 客户名称
                $sheet->setCellValue('M'.($k+2),$v['skfs_name']); // 收款方式
			}

			$filename = '交易明细.'.config('my.dump_extension');
