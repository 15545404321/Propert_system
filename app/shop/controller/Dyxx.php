...
/*
 module:		单元信息控制器
 create_time:	2023-03-14 18:39:56
 author:		
 contact:		
*/
...
use app\shop\model\Dyxx as DyxxModel;
use think\facade\Db;

class Dyxx extends Admin {
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$where['a.louyu_pid'] = $this->request->post('louyu_pid', '', 'serach_in');

			$order  = $this->request->post('order', '', 'serach_in');	//排序字段
			$sort  = $this->request->post('sort', '', 'serach_in');		//排序方式

			$orderby = ($sort && $order) ? $sort.' '.$order : 'louyu_id';
...
where louyu_pid is not Null";
	...
	public function add(){
		...
		$this->validate($data,\app\shop\validate\Dyxx::class);
		...
		try{
			$res = DyxxModel::insertGetId($data);
		}catch(\Exception $e){
			throw new ValidateException($e->getMessage());
		}

		if($ret = hook('hook/Dyxx@afterShopAdd',array_merge($data,['louyu_id'=>$res]))){
			return $ret;
		}
	...
	function getUpdateInfo(){
		...
		$res = DyxxModel::field($field)->find($id);
	...
	public function jiaDy(){
		...
		$res = DyxxModel::field('louyu_dysl')->where('louyu_id',$data['louyu_id'])->inc('louyu_dysl',$data['louyu_dysl'])->update();

		if($ret = hook('hook/Dyxx@afterShopJiaDy',$data)){
			return $ret;
		}
	...
	public function jiaLc(){
		...
		if($ret = hook('hook/Dyxx@beforShopJiaLc',$data)){
			return $ret;
		}

		$res = DyxxModel::field('louyu_lczs')->where('louyu_id',$data['louyu_id'])->inc('louyu_lczs',$data['louyu_lczs'])->update();

		if($ret = hook('hook/Dyxx@afterShopJiaLc',$data)){
			return $ret;
		}
	...
	function delete(){
		...
		if($ret = hook('hook/Dyxx@beforShopDelete',$idx)){
			return $ret;
		}

		DyxxModel::destroy(['louyu_id'=>explode(',',$idx)],true);

		if($ret = hook('hook/Dyxx@afterShopDelete',$idx)){
			return $ret;
		}
