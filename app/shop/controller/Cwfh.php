...
/*
 module:		财务复核控制器
 create_time:	2023-02-01 16:47:00
 author:		
 contact:		
*/
...
use app\shop\model\Cwfh as CwfhModel;
use think\facade\Db;

class Cwfh extends Admin {
	...
	function initialize(){
		parent::initialize();
		if(in_array($this->request->action(),['detail','delete','CwfhUpdate','getCwfhUpdateInfo'])){
			$idx = $this->request->post('yssj_id','','serach_in');
			if($idx){
				foreach(explode(',',$idx) as $v){
					$info = CwfhModel::find($v);
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$where['a.yssj_cwsh'] = $this->request->post('yssj_cwsh', '', 'serach_in');
...
left join cd_cwqy as w on w.cwqy_id = f.cwqy_id";
	...
	function delete(){
		...
		if($ret = hook('hook/Cwfh@beforShopDelete',$idx)){
			return $ret;
		}

		CwfhModel::destroy(['yssj_id'=>explode(',',$idx)],true);

		if($ret = hook('hook/Cwfh@afterShopDelete',$idx)){
			return $ret;
		}
 	...
 	* @Description  财务审核
 	*/
	public function CwfhUpdate(){
		$postField = 'yssj_id,yssj_cwsh';
		$data = $this->request->only(explode(',',$postField),'post',null);

		$this->validate($data,\app\shop\validate\Cwfh::class);

		try{
			CwfhModel::update($data);
		}catch(\Exception $e){
 	...
 	*/
	function getCwfhUpdateInfo(){
		$id =  $this->request->post('yssj_id', '', 'serach_in');
		if(!$id) throw new ValidateException ('参数错误');
		$field = 'yssj_id,yssj_cwsh';
		$res = CwfhModel::field($field)->find($id);
